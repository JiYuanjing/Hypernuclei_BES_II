...
void drawerrbox(double x, double y, double ysys, double xwidth,  int color,int fillstyle,int mode=0){
  ...
  // box->SetLineWidth(2);
  if (mode==0) box->SetFillColorAlpha(color,0.4);
  else if (mode==1) {
    box->SetFillStyle(0);
...
void drawGraphWithSys(TGraphErrors* g,TGraphErrors* gsys , int color,int style,float size,int mod=0,int fillcolor=0,double xwidth=0.025,double yheight = 0.005)
{
  // TFile* f = TFile::Open(fname.Data());
  // TGraphErrors* g = (TGraphErrors*)f->Get(gname.Data());
  // TGraphErrors* gsys = (TGraphErrors*)f->Get(gnamesys.Data());
  ...
  for (int ip=0;ip<npoints;ip++)
  {
    ...
    if (mod==0) drawerrbar(x,y,yerr,xwidth,yheight,color); 
    else if (mod==1) drawerrbox(x,y,yerr,xwidth,color,1001);
    else if (mod==2) drawerrbox(x,y,yerr,xwidth,color,1001,1);
